...
void VoxbloxGroundTruthPlugin::Load(physics::WorldPtr world,
  ...
  std::string service_name_tsdf = "save_voxblox_ground_truth_to_tsdf";
  std::string service_name_pcd = "save_voxblox_ground_truth_to_pcd";
  std::string service_name_ply = "save_voxblox_ground_truth_to_ply";
  LOG(INFO) << "Advertising service: " << service_name_tsdf;
  LOG(INFO) << "Advertising service: " << service_name_pcd;
  LOG(INFO) << "Advertising service: " << service_name_ply;
  
  srv_tsdf = nh_private_.advertiseService(service_name_tsdf, &VoxbloxGroundTruthPlugin::serviceCallbackTSDF, this);
  srv_pcd = nh_private_.advertiseService(service_name_pcd, &VoxbloxGroundTruthPlugin::serviceCallbackPCD, this);
  srv_ply = nh_private_.advertiseService(service_name_ply, &VoxbloxGroundTruthPlugin::serviceCallbackPLY, this);
}

bool VoxbloxGroundTruthPlugin::serviceCallbackTSDF(
...
bool VoxbloxGroundTruthPlugin::serviceCallbackPCD(
    voxblox_msgs::FilePath::Request& request,
    voxblox_msgs::FilePath::Response& response) {
  // Read the voxel size from ROS params
  CHECK(nh_private_.getParam("/voxblox_ground_truth/voxel_size", voxel_size_))
...
bool VoxbloxGroundTruthPlugin::serviceCallbackPLY(
    voxblox_msgs::FilePath::Request& request,
    voxblox_msgs::FilePath::Response& response) {
  // Read the voxel size from ROS params
  CHECK(nh_private_.getParam("/voxblox_ground_truth/voxel_size", voxel_size_))
  ...
  // Save the PLY to a file
  LOG(INFO) << "Saving PLY to file: " << request.file_path;
  ...
  pcl::io::savePLYFileASCII<pcl::PointXYZ> (request.file_path, pointcloud_out);
